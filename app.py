...
st.markdown(
    "<div style='text-align: center; font-size: 100px; font-family: Georgia, serif'>CREATIVE</div>",
        ...
        for i, url in enumerate(video_urls):
            ...
            if video_id:
                ...
                if transcript:
                    ...
                    truncated_transcript = truncate_text(
                        formatted_transcript, max_length=4000
                    )
                    ...
                    st.warning(
                        f"{analysis_texts[i]}에 대한 대본을 가져올 수 없습니다. 자막이 비활성화되어 있을 수 있습니다."
                    )
...
if st.session_state.combined_recommendations:
    ...
    if st.button("저장하기: 종합 추천 주제"):
        save_to_local_storage(
            "combined_recommendations", st.session_state.combined_recommendations
        )
...
# 사이드바에 방문자 수 표시
st.sidebar.markdown(
    "<h3 style='font-size:10px;'>방문자 수</h3>", unsafe_allow_html=True
)
st.sidebar.markdown(
    f"<p style='font-size:10px;'>Today: {st.session_state['visitor_count']}</p>",
    unsafe_allow_html=True,
)
